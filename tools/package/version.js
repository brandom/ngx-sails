...
i
  .prompt({ name: 'version', message: 'Choose a version:', type: 'list', choices: versionList })
  .then(answer => {
...
function getVersionInput() {
  i.prompt([ { name: 'version', message: 'New version:' } ]).then(answer => {
    ...
    if (!semver.gt(version, packageJSON.version)) {
      console.error(
        `Version "${version}" must be greater than the current version "${packageJSON.version}"`
      );
      return getVersionInput();
    }
    writeVersion(version);
...
function writeVersion(version) {
  spawnSync('node_modules/.bin/json', [
    '-I',
    '-f',
    'package.json',
    '-e',
